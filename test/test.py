...
async def test_project_0d(dut):
    ...
    for sub_i_I_cor in range(8):
        ...
        await ClockCycles(dut.clk, int(1600000/50))
        
        dut.rst_n.value = 1
        select="001"
        ui_valor=int(I_cor+select,2)
        dut.ui_in.value = ui_valor
        await ClockCycles(dut.clk, int(80000/50))
        dut.rst_n.value = 0
        await ClockCycles(dut.clk, int(1600000/50))
